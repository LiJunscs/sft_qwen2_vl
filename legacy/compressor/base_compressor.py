...
class S2Compressor(torch.nn.Module):
    compressor_name: str = "2x2_S2"

    def __init__(self, context_dim: int, **kwargs):
        super().__init__()
        self.hidden_size = context_dim * 4

        self.ln_q = LayerNorm(self.hidden_size, eps=1e-6)
        self.mlp = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size),
            nn.GELU(),
            nn.Linear(self.hidden_size, context_dim),
        )
    ...
    def forward(self, vit_embeds: torch.Tensor, grid_thw: torch.Tensor):
        ## NOTE: 由于每个图片或者视频的thw不一定相同，因此暂时采用循环完成
        last_visual_end = 0
        max_padding_length = 0
        num_visuals = grid_thw.shape[0]
        embeds_compressed = []
        for i in range(num_visuals):
            t, h, w = grid_thw[i]
            embeds_i = vit_embeds[last_visual_end : last_visual_end + t * h * w]
            embeds_i, thw = self.flat_square_2x2(embeds_i, grid_thw[i])

            embeds_compressed.append(embeds_i)
            max_padding_length = max(max_padding_length, embeds_i.shape[0])
            grid_thw[i] = thw
            last_visual_end = last_visual_end + t * h *w
        embeds_compressed = torch.cat(embeds_compressed, dim=0) # shape (sum(compressed_visual_length, self.hidden_size))

        embeds_compressed = self.mlp(embeds_compressed)
        embeds_compressed = embeds_compressed.contiguous()
        return embeds_compressed, grid_thw
